...
async function main () {
  ...
  const FlashArbitrageBot = await hre.ethers.getContractFactory('FlashArbitrageBot')
  const flashArbitrageBot = await FlashArbitrageBot.deploy(AAVE_POOL_ADDRESSES_PROVIDER)
  ...
  if (hre.network.name !== 'hardhat') {
    ...
    try {
      await hre.run('verify:verify', {
        ...
        constructorArguments: [UNISWAP_V2_ROUTER, UNISWAP_V3_ROUTER, SUSHISWAP_ROUTER]
      })
  ...
  console.log('\n=== NEXT STEPS ===')
  console.log('1. Update frontend contract addresses in src/hooks/useArbitrageContract.ts')
  console.log('2. Fund the FlashArbitrageBot contract with initial ETH for gas')

...
const TradingInterface: React.FC = () => {
  const { tradingConfig, setTradingConfig, botActive, setBotActive } =
    useStore();

  const [activeTab, setActiveTab] = React.useState<
    "config" | "backtest" | "calculator" | "risk" | "contracts"
  >("config");
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      if (e.target.checked) {
                        setTradingConfig({
                          selectedTokens: [
                            ...tradingConfig.selectedTokens,
                            token,
                          ],
                        });
                      } else {
                        setTradingConfig({
                          selectedTokens: tradingConfig.selectedTokens.filter(
                            (t) => t !== token,
                          ),
                        });
                      }
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      if (e.target.checked) {
                        setTradingConfig({
                          selectedExchanges: [
                            ...tradingConfig.selectedExchanges,
                            exchange,
                          ],
                        });
                      } else {
                        setTradingConfig({
                          selectedExchanges:
                            tradingConfig.selectedExchanges.filter(
                              (ex) => ex !== exchange,
                            ),
                        });
                      }
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg p-6 border border-gray-200 dark:border-gray-700">
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                  Max Transaction Amount: {tradingConfig.maxTransactionAmount}{" "}
                  ETH
                </label>
                <input
                  type="range"
            ...
            <div className="space-y-4">
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setTradingConfig({ gasStrategy: e.target.value as any })
                  }
              ...
              <div className="flex items-center space-x-2">
                <input
                  ...
                  onChange={(e) =>
                    setTradingConfig({ autoExecute: e.target.checked })
                  }
                  className="rounded border-gray-300 text-primary-500 focus:ring-primary-500"
                />
                <label
        ...
        <div className="flex items-start space-x-3">
          ...
          <div>
            ...
            <p className="text-sm text-yellow-700 dark:text-yellow-300 mt-1">
              Flash loan arbitrage involves significant financial risk. Only
              trade with funds you can afford to lose. Market conditions can
              change rapidly, and trades may fail due to network congestion,
              slippage, or MEV attacks.
            </p>
          </div>
        </div>
      </div>

...
const NetworkSelector: React.FC = () => {
  ...
  const currentNetwork =
    networks.find((n) => n.id === wallet.network?.chainId) || networks[0];
    ...
    try {
      ...
      if (switchError.code === 4902) {
        try {
          ...
          alert(
            `Failed to add ${network.name} network. Please add it manually in MetaMask.`,
          );
        }
        ...
        alert(
          `Failed to switch to ${network.name}. Please try again or switch manually in MetaMask.`,
        );
      }
    } finally {
          ...
          <div className="absolute right-0 mt-2 w-56 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 z-50">
            <div className="p-2">
              ...
              {networks.map((network) => (
                ...
                >
                  <div
                    className={`w-4 h-4 rounded-full ${network.color} flex-shrink-0`}
                  />

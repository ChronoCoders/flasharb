...
export const useRealTimeData = () => {
    ...
    async (tokens: string[]) => {
      try {
        ...
        try {
              ...
              tokenData.pairs.forEach((pair: any) => {
                if (pair.dexId && pair.priceUsd) {
                  const exchangeName =
                    pair.dexId === "uniswap"
                      ? "Uniswap V2"
                      : pair.dexId === "uniswapv3"
                        ? "Uniswap V3"
                        : pair.dexId === "sushiswap"
                          ? "SushiSwap"
                          : pair.dexId;

                  priceMap[tokenData.token].exchanges[exchangeName] =
          ...
          console.warn(
            "DEX price fetch failed, continuing with CoinGecko data only:",
            dexError,
          );
        }
  ...
  const detectArbitrageOpportunities = useCallback(
        ...
        for (let i = 0; i < exchanges.length; i++) {
          for (let j = i + 1; j < exchanges.length; j++) {
            ...
            const priceDifferencePercent =
              (priceDifference / Math.min(priceA, priceB)) * 100;
            ...
            if (priceDifferencePercent > 0.1) {
              ...
              const potentialProfit =
                (priceDifference / Math.min(priceA, priceB)) *
                tradeAmount *
              ...
              const gasEstimate =
                (gasPrice * gasUnits * tokenPrice.price) / 1e9; // Convert gwei to USD

              const netProfit = potentialProfit - gasEstimate;

              if (netProfit > 5) {
                // Minimum $5 profit threshold
                opportunities.push({
  ...
  useEffect(() => {
    const initializeRealData = async () => {
      ...
      try {
        ...
        if (!API_KEYS.etherscan) {
          throw new Error(
            "Etherscan API key is required. Please add VITE_ETHERSCAN_API_KEY to your .env file.",
          );
        }
        ...
        const initialOpportunities =
          detectArbitrageOpportunities(currentPrices);
        ...
        setError(
          error instanceof Error
            ? error.message
            : "Failed to connect to real-time data sources",
        );
        setIsLoading(false);
      }
    };
  ...
  const refreshData = useCallback(async () => {
    ...
    try {
      ...
      setError(
        error instanceof Error
          ? error.message
          : "Failed to refresh real-time data",
      );
    } finally {
      setIsLoading(false);
    }

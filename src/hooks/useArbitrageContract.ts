...
export const useArbitrageContract = () => {
  ...
  const [contractBalance, setContractBalance] = useState<Record<string, number>>({});
  ...
  const getContract = useCallback((address: string, abi: string[]) => {
    const provider = getProvider();
    return new ethers.Contract(address, abi, provider);
  }, [getProvider]);

  const getSignedContract = useCallback(async (address: string, abi: string[]) => {
    const provider = getProvider();
  ...
  const executeArbitrage = useCallback(async (opportunity: ArbitrageOpportunity) => {
    ...
    try {
      ...
      const params = {
        ...
        minProfit: ethers.parseEther((opportunity.netProfit * 0.95).toString()) // 5% slippage on profit
      };
  ...
  const getContractBalance = useCallback(async (tokenAddress: string) => {
    try {
  ...
  const withdrawProfits = useCallback(async (tokenAddress: string, amount: string) => {
    try {
      const arbitrageContract = await getSignedContract(
        CONTRACT_ADDRESSES.arbitrageBot,
        ARBITRAGE_ABI
...
const generateSwapData = async (opportunity: ArbitrageOpportunity): Promise<string[]> => {
